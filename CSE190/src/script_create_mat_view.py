...
cursor.execute("CREATE TABLE mat_view_post (posterID int NOT NULL, num_of_post int NOT NULL, PRIMARY KEY (posterID), FOREIGN KEY (posterID) REFERENCES member)")
cursor.execute("CREATE INDEX mat_view_post_index ON mat_view_post (posterID)")
...
cursor.execute("CREATE TABLE mat_view_case2 (readerID int NOT NULL, friendID int NOT NULL, nation varchar(30), num_of_read int NOT NULL, PRIMARY KEY (readerID, friendID, num_of_read), FOREIGN KEY (readerID) REFERENCES member, FOREIGN KEY (friendID) REFERENCES member)")
